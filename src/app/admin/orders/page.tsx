...
export default function AdminOrdersPage() {
  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        // Mapear order_items a items y users a user para que OrderCard lo encuentre
        const mappedOrders = data.orders?.map((order: any) => ({
          ...order,
          items: order.order_items || [],
          // Supabase devuelve 'users' como array aunque sea relación 1-1, tomar el primer elemento
          user: Array.isArray(order.users) ? order.users[0] : (order.users || order.user),
        })) || []
  ...
  useEffect(() => {
    const filtered = orders.filter((order) => {
      ...
      // Convertir toda la orden a string JSON y buscar en ella
  ...
  return (
    <AdminProtector>
      ...
      <div className="mb-6">
        <Input
          type="text"
          placeholder="🔍 Buscar por nombre, email, cédula u ID de orden..."
          value={searchTerm}
